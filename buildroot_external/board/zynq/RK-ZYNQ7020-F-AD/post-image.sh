...
# The AD9361 fabric (axi_ad9361/dmac/sysid) only shows up on the AXI
# bus once the PL is configured. U-Boot's CONFIG_PREBOOT loads
...
# PL-backed nodes just won't probe), but warn loudly so it isn't
# missed silently in CI.
if [ -s "${BOARD_DIR}/fpga/system.bit.bin" ]; then
	...
	echo "WARNING: shipping a 0-byte placeholder - AD9361/PL will not work until you supply the real bitstream and rebuild." >&2
