...
#define BL808_I2C_CONFIG_SUB_ADDR_BC_MASK (0x3 \
                                           << BL808_I2C_CONFIG_SUB_ADDR_BC_SHIFT)
#define BL808_I2C_CONFIG_10B_ADDR_EN BIT(7)
/* target addr */
#define BL808_I2C_CONFIG_SLV_ADDR_SHIFT UL(8)
#define BL808_I2C_CONFIG_SLV_ADDR_MASK (0x3ff \
                                        << BL808_I2C_CONFIG_SLV_ADDR_SHIFT)
/* packet length in byte */
#define BL808_I2C_CONFIG_PKT_LEN_SHIFT UL(20)
#define BL808_I2C_CONFIG_PKT_LEN_MASK (0xff \
                                       << BL808_I2C_CONFIG_PKT_LEN_SHIFT)
/* de-glitch function cycle count */
#define BL808_I2C_CONFIG_DEG_CNT_SHIFT UL(28)
#define BL808_I2C_CONFIG_DEG_CNT_MASK (0xf \
                                       << BL808_I2C_CONFIG_DEG_CNT_SHIFT)
...
#define BL808_I2C_FIFO_CONFIG_1_TX_FIFO_CNT_MASK (0x3 \
                                                  << BL808_I2C_FIFO_CONFIG_1_TX_FIFO_CNT_SHIFT)
#define BL808_I2C_FIFO_CONFIG_1_RX_FIFO_CNT_SHIFT UL(8)
#define BL808_I2C_FIFO_CONFIG_1_RX_FIFO_CNT_MASK (0x3 \
                                                  << BL808_I2C_FIFO_CONFIG_1_RX_FIFO_CNT_SHIFT)
                                  ...
                                  unsigned long parent_rate)
{
    ...
    {
        divider = 0xff;
        dev_warn(dev, "requested rate %lu is slower than minmum, "
                      "setting to slowest possible rate\n",
                 rate);
    }
...
static void bl808_i2c_disable_interrupts(struct bl808_i2c_dev *i2c_dev,
                                         u32 irqs)
{
    u32 val;
...
static int bl808_i2c_start_transfer(struct bl808_i2c_dev *i2c_dev)
{
    ...
    bl808_i2c_addr_config(i2c_dev, msg->addr, subaddr, subaddr_size,
                          is_ten_bit);
...
static irqreturn_t bl808_i2c_isr(int this_isq, void *data)
{
    ...
    else if (val & BL808_I2C_STS_RXF_INT)
    {
        ...
        {
            dev_err(dev,
                    "wants receive data to be popped, but no where to put\n");
            i2c_dev->msg_err = -EREMOTEIO;
            goto complete;
        }
